...
export default function Contact() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      try {
        const token = await grecaptcha.execute("6Lch_WcrAAAAAHC1MmgVuDefEhOQSxb573Tk7DMM", { action: 'submit' });
        // proceed with token
      } catch (error) {
      ...
      if (!token) {
        setStatus({ type: "error", message: "reCAPTCHA validation failed." });
        return;
      }
      ...
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ ...form, token }), // send token if backend verifies it
      });
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 sm:px-10 md:px-16 lg:px-24 text-black">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <ReCAPTCHA sitekey="6Lch_WcrAAAAAHC1MmgVuDefEhOQSxb573Tk7DMM" size="invisible" ref={recaptchaRef} />
